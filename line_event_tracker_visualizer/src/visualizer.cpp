...
void Visualizer::imageCallback(const sensor_msgs::Image::ConstPtr& msg)
{
...

  {
    ...
    for (auto const &line: lines_.lines) {
      ...
      auto mid_point = cv::Point2d(line.u_l, line.v_l);
      auto end_point_1 = cv::Point2d(line.u_l + line.length * std::sin(line.theta) / 2,
                                     line.v_l + line.length * std::cos(line.theta) / 2);
      auto end_point_2 = cv::Point2d(line.u_l - line.length * std::sin(line.theta) / 2,
                                     line.v_l - line.length * std::cos(line.theta) / 2);
